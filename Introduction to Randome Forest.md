...
순서는 크게 중요하지 않지만, 한 지점에서 분할하는 의사 결정 트리를 만들 것이기 때문에(예: `High` vs. `Low` 와 `Medium` , `High` 와 `Low` vs. `Medium`) 약간 이상하죠. 
...
캐글대회의 공개이사회를 보면 0.25의 RMSE가 상위 25% 안팎으로 떨어집니다. 
랜덤 포레스트는 매우 강력하며, 이 완전히 표준화된 프로세스는 모든 데이터셋에 매우 유용합니다.
